...
DEPEND="
	...
	fuse? ( sys-fs/fuse:3= )
	jaeger? (
		>=dev-cpp/opentelemetry-cpp-1.24.0:=[otlp,http,prometheus]
		net-misc/curl
	)
...
PATCHES=(
	...
	# OTLP HTTP exporter migration (replaces system-otel and jaeger exporter)
	"${FILESDIR}/ceph-20.1.1-otlp-cmake.patch"
	"${FILESDIR}/ceph-20.1.1-otlp-tracer.patch"
	"${FILESDIR}/ceph-20.1.1-otlp-config.patch"
	...
	"${FILESDIR}/ceph-20.1.1-tracer-cross-osd-bluestore.patch"
	"${FILESDIR}/ceph-20.1.1-osd-peering-perfcounters.patch"
	...
	# bluestore tracer must bind to global provider, not create its own
	# (otherwise the second SetTracerProvider call orphans the osd exporter)
	"${FILESDIR}/ceph-20.1.1-bluestore-tracer-init-from-global.patch"
	...
	# kstore::read override also needs the parent_trace param added
	# to the ObjectStore base by bluestore-read-subspans (gcc 15 hard error)
	"${FILESDIR}/ceph-20.1.1-kstore-read-parent-trace.patch"
	# build-fix for above: ECSwitch+ECBackend{,L} need parent_trace too,
	# and PrimaryLogPG.cc must explicit-init jspan_context (opentelemetry-cpp 1.24)
	"${FILESDIR}/ceph-20.1.1-bluestore-read-subspans-buildfix.patch"
	...
	# librados::async_operate read lambda must also forward trace_ctx to the
	# 6-arg aio_operate (signature was updated but lambda body was not)
	"${FILESDIR}/ceph-20.1.1-librados-asio-read-trace.patch"
	"${FILESDIR}/ceph-20.1.1-ec-backendl-otel-trace-propagation.patch"
	# rados_read span around each librados chunk read in rgw_aio.cc
	# (S3 GET data path doesn't go through rgw_rados_operate)
	"${FILESDIR}/ceph-20.1.1-rgw-aio-rados-read-span.patch"
	# When trace_ctx is null/invalid (most real call sites), fall back to
	# start_trace so rados_read still emits as an orphan span
	"${FILESDIR}/ceph-20.1.1-rgw-aio-rados-read-startfallback.patch"
	# Final wire-up: rgw_op.cc sets ReadOp::params.trace_ctx so the rados_read
	# span actually links under the rgw GET trace (otherwise orphan)
	"${FILESDIR}/ceph-20.1.1-rgw-iterate-trace-ctx-plumbing.patch"
	# Per-phase bluestore write spans (_do_write_data, _do_write_small/big,
	# kv_submit_transaction, kv_committed_finalize) under queue_transactions
	"${FILESDIR}/ceph-20.1.1-bluestore-write-phase-spans.patch"
)
