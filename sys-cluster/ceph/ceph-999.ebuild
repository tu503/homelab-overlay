...
inherit check-reqs bash-completion-r1 cmake flag-o-matic git-r3 lua-single multiprocessing \
		python-r1 udev readme.gentoo-r1 toolchain-funcs systemd tmpfiles

DESCRIPTION="Ceph distributed filesystem (homelab fork with OTLP tracing)"
...
SRC_URI="
	parquet? ( https://github.com/xtensor-stack/xsimd/archive/13.0.0.tar.gz -> ceph-xsimd-20.1.1.tar.gz
...
PATCHES=(
	...
	# OTLP/tracer patches not needed — already committed in the ceph-999 branch
	# (rgw multipart span name fix, bluestore tracer init + read span,
	#  librados read jspan_context overload, bluestore read sub-spans —
	#  see commits 452dbad..84be011 on tu503/ceph ceph-999)
...
src_unpack() {
	git-r3_src_unpack
	# Unpack additional archives (parquet xsimd/arrow tarballs)
	if use parquet; then
		unpack ${A}
	fi
}
...
ceph_src_configure() {
	...
	use parquet && export ARROW_XSIMD_URL="file:///${DISTDIR}/ceph-xsimd-20.1.1.tar.gz"
